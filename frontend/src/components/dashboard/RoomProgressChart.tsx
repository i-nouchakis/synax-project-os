...
const STATUS_CONFIG = [
  ...
  { key: 'blocked', label: 'Blocked', color: CHART_COLORS.danger },
] as const;

export function RoomProgressChart({ data }: Props) {
  const [hovered, setHovered] = useState<string | null>(null);

  if (!data) return null;

  const total = data.notStarted + data.inProgress + data.completed + data.blocked;
  ...
  if (total === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[220px] text-text-tertiary">
        <p className="text-body-sm">No rooms</p>
      </div>
    );
  }
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            {hoveredSeg ? (
              ...
              <>
                ...
                <span className="text-caption text-text-tertiary">{data.completed}/{total}</span>
              </>
            )}
          </div>
        </div>
